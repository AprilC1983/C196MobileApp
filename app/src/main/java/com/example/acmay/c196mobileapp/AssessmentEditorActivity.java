...
import static com.example.acmay.c196mobileapp.utilities.Constants.DUE_CHANNEL_ID;
...
public class AssessmentEditorActivity extends AppCompatActivity {
    ...
    private void createAlert(long date, String message) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            CharSequence name = getString(R.string.due_channel_name);
            String description = getString(R.string.due_channel_description);
            int importance = NotificationManager.IMPORTANCE_DEFAULT;
            NotificationChannel channel = new NotificationChannel(DUE_CHANNEL_ID, name, importance);
...
//Specifies which screen launches***********************************************************************8
            Intent intent = new Intent(this, DueAlertActivity.class);
            intent.putExtra(DUE_ALERT_MESSAGE_KEY, message);
...
//This will trigger an alert for start and end dates
    ...
    private void saveAndReturn() {
        String title = "";
        title = assessmentTextView.getText().toString();
        String type = "";
        String message = title + " is due today";
